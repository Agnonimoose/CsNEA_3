...
class Character(pygame.sprite.Sprite):
    ...
    def check_collisions(self, platforms, camera_offset):
        ...
        if self.on_ground and self.velocity_y == 0:
            print("standing on, going to check falling")
            if self.check_falling_off() == True:
                self.on_ground = False
                print("falling started!")
                self.standing_on = None
        else:
            print("not standing on!")
...
class World:
    ...
    def renderWorld(self, BUFFER, offset = (0, 0)):
        ...
        for x in range(offset[0] // self.tile_size, (offset[0] + BUFFER.get_width()) // self.tile_size + 1):
            for y in range(offset[1] // self.tile_size, (offset[1] + BUFFER.get_height()) // self.tile_size + 1):
                loc = str(x) + ';' + str(y)
                if loc in self.tilemap:
                    tile = self.tilemap[loc]
                    BUFFER.blit(self.game.assets[tile['type']][tile['variant']], (
